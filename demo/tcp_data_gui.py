...
POSITIONS_CSV = 'flight_positions.csv'
TIMETABLE_CSV = 'flight_timetable.csv'
CYCLE_CSV = 'flight_cycle.csv'
...
class DataGUI(tk.Tk):
    ...
    def _load_data(self):
        try:
            df_pos = pd.read_csv(os.path.join(BASE_DIR, POSITIONS_CSV))
            df_tt = pd.read_csv(os.path.join(BASE_DIR, TIMETABLE_CSV))
            df_cyc = pd.read_csv(os.path.join(BASE_DIR, CYCLE_CSV))
    ...
    def cmd_listen(self):
        ...
        def start():
            ...
            try:
                ...
                while self.running and not self.cmd_conn:
    ...
    def cmd_start_push(self):
        if self.data_sock is None:
            self._log('error', '尚未连接三维端数据通道，请先点击“连接三维端”')
            return
    ...
    def cmd_pause_push(self):
        ...
        self.after(0, lambda: self.btn_start.config(text='▶ 继续', state='normal'))
    ...
    def _build_timetable(self, ft, records):
        states = [{'trackId': r['trackId'], 'state': r['state'], 'battery': r['battery'], 'speed': r['speed']}
                  for r in records]
